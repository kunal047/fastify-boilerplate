...
function factory(options) {
    ...
    const {
        keys, errorResponse, contentType, bearerType, auth, auth_header,
    } = _options;
    function bearerAuthHook(fastifyReq, fastifyRes, next) {
        const header = fastifyReq.req.headers[auth_header];
        if (!header) {
        ...
        Promise.resolve(retVal).then((val) => {
            ...
            fastifyRes.code(500)
                .send(errorResponse(err instanceof Error ? err : Error(String(err))));
        });
    }
